...
export class FileSystem {
  ...
  async getLocalFilePathFromUrl(url, onProgress, permanent) {
    let filePath = null;
    ...
    if (exists[0]) {
      ...
      let result = await this.fetchFile(url, onProgress, permanent, null, true); // Clobber must be true to allow concurrent CacheableImage components with same source url (ie: bullet point images).
   ...
   */
  async fetchFile(url, onProgress, permanent = false, fileName = null, clobber = false) {
